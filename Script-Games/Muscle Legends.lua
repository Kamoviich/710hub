-- 710Hub - Muscle Legends (community rebuild, 2026)
-- Keeps the original 710Hub loader format: GameList.lua points to this module.
...
if game.GameId ~= 1268927906 then return end
...
-- Prevent duplicate copies after re-executing the loader.
...
local function equipBestOwned()
    ...
    -- Unequip currently equipped pets, then equip the strongest owned pets.
...
task.spawn(function()
    while task.wait(2) do
        if S.Chests then
            for _,name in ipairs(CHESTS) do safeInvoke(R.Chest, name); task.wait(.12) end
        end
    end
end)
...
local function button(text, cb)
    ...
    b.Font=Enum.Font.GothamMedium b.TextSize=14 b.Text=text
...
local function section(t)
    ...
    l.Size=UDim2.new(1,-6,0,28) l.BackgroundTransparency=1
    l.Text=t l.TextXAlignment=Enum.TextXAlignment.Left
...
button("Hatch x1",function() safeInvoke(R.Crystal,"openCrystal",S.HatchCrystal) end)
...
if area then
    for _,obj in ipairs(area:GetDescendants()) do
        if obj:IsA("BasePart") then tpNames[#tpNames+1]=obj.Name; tpParts[obj.Name]=obj end
    end
    table.sort(tpNames)
end
local tpIndex=1
...
button("Go To Selected Teleport",function()
    ...
    if p and c and c:FindFirstChild("HumanoidRootPart") then c.HumanoidRootPart.CFrame=p.CFrame+Vector3.new(0,4,0) end
end)

section("UTILITY")
button("Stop All",function()
    S.Train=false S.Rebirth=false S.Chests=false S.Hatch=false S.Brawl=false
end)
button("Close UI",function() gui:Destroy() end)

print("[710Hub] Muscle Legends module loaded")
